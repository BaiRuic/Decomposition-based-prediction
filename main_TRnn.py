...
from model.TRcn_Based import stack
...
class Train():
    ...
    def save_state(self, state,filename=path + "\model\TRcn_Based\my_state.pt"):
        my_log.info_logger("Saving model and optimizer state")
        torch.save(state, filename)

    def load_state(self, filename=path + "\model\TRcn_Based\my_state.pt"):
...
def main():
    ...
    model = stack.Stack(input_size=HyperParams['features'], 
                        hidden_size_list=[4,5,6,7,8], 
                        input_seqlen=HyperParams['time_steps'], 
